...
sites = {
        ...
        "bofa": {
            "caps": [ "SIGNON", "BASTMT" ],
            "fid": "6812",     # ^- this is what i added, for checking/savings/debit accounts- think "bank statement"
            "fiorg": "HAN", 
            "url": "https://ofx.bankofamerica.com/cgi-forte/fortecgi?servicename=ofx_2-3&pagename=ofx",
            "bankid": "011000138", # bank routing #
            }	
        }
...
class OFXClient:
    ...
    def doQuery(self,query,fileName):
        open(fileName, 'w').write(self.getResponse(query))

def getOfxResponse(config, dtstart):
    dtstartstr = dtstart.strftime('%Y%m%d')
    client = OFXClient(config, config['user'], config['password'])
    acctType = config['acctType']
    if acctType in ('CHECKING', 'SAVINGS', 'MONEYMRKT'):
        print 'Making bank query for %s account' % acctType
        query = client.baQuery(config['acctId'], dtstartstr, config['acctType'])
    elif acctType == 'CCSTMT':
        print 'Making credit card query'
        query = client.ccQuery(config['acctId'], dtstartstr)
...
if __name__=="__main__":
    dtstart = "20100101" #time.strftime("%Y%m%d",time.localtime(time.time()-31*86400))
